...
def test_bitonic_sort_invalid_input():
    """Test bitonic sort with invalid input type"""
    with pytest.raises(TypeError):
        bitonic_sort("not a list")

def test_bitonic_sort_with_mixed_types():
    """Test bitonic sort with comparable mixed types"""
    arr = [3, 1.5, 4, 1, 5.5]
    result = bitonic_sort(arr)
    assert result == [1, 1.5, 3, 4, 5.5]